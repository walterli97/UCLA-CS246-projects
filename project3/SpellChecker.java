...
public class SpellChecker {
    ...
    public SpellChecker(String dictionaryFile) throws IOException {
        ...
        for (String key : dict.keySet())    totalWords += dict.get(key);
    }
    ...
    public double calculateLM(String word) {
        ...
        //
        return dict.getOrDefault(word, 0) / (totalWords + 0.0);
    ...
    */
    private Map < String, Double > EditDist1(String word) {
        ...
        for (int i = 0; i <= word.length(); ++i) {
            ...
            if (R.length() > 1) {
                temp = L + R.charAt(1) + R.charAt(0) + R.substring(2);
                cands.put(temp, 1.0);
            }
            ...
            if (R.length() > 0) {
                for (int j=0; j<alphabet.length(); ++j) {
                    if (alphabet.charAt(j) != R.charAt(0)) {
